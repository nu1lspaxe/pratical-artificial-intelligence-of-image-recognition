...
for i in range(0,12): #讀取0~11號 colormap
    ...
    ax= plt.imshow(im2, cmap= colormaps[i], vmin=50, vmax=150) #用第i的colormap套色
    plt.title(colormaps[i], y=-0.2, fontsize= 16) #用colormap的名稱做標題,標題在圖的下方
    plt.axis('off')
    plt.colorbar(ax, extend='both') #標示 colorbar, 兩端尖化
